...
{
	ChipLogProgress(Zcl, "MatterPostAttributChangeCallback, value%d\n", *value);
	...
	if (path.nAttributeId != BooleanState::Attributes::StateValue::Id)
	{
	...
	else
	{
		task.PostContactActionRequest(*value ? ContactSensorManager::Action::kSignalDetected
											 : ContackSensorManager::Action::kSignalLost);
	}
}
 ...
 */
 ...
 void emberAfBooleanSensorConfigurationClusterInitCallback(EndpointId endpoint)
 {
 ...
 void LogBooleanStateEvent(bool state)
 {
 	...
 	{
 		ChipLogProgress(Zcl, "booleanstate: failed to record state-change event");
 	}
 }
